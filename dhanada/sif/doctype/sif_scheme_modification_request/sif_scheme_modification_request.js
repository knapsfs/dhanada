...
frappe.ui.form.on("SIF Scheme Modification Request", {
		...
		const format_json_field = (value, doc) => {
			...
			if (doc.field_name === "managers") {
				try {
					...
					arr.forEach((m) => {
						const name =
							(frm.manager_name_map && frm.manager_name_map[m.manager_name]) ||
							m.manager_name;
						let details = name;
						if (m.from_date)
							details += ` &mdash; From: ${frappe.datetime.str_to_user(
								m.from_date
							)}`;
						if (m.to_date)
							details += ` &mdash; To: ${frappe.datetime.str_to_user(m.to_date)}`;
			...
			try {
				...
				return `<pre style="font-size: 11px; max-height: 200px;">${JSON.stringify(
					obj,
					null,
					2
				)}</pre>`;
	...
	apply_dynamic_options(frm) {
		...
		let base_df = frappe.meta.get_docfield(
			"SIF Scheme Modification Request Item",
			"field_name"
		);
		if (base_df) base_df.options = options;

		let meta_df = frappe.meta.get_docfield(
			"SIF Scheme Modification Request Item",
			"field_name",
			frm.doc.name
		);
		...
		if (frm.fields_dict.changed_fields && frm.fields_dict.changed_fields.grid) {
			try {
				frm.fields_dict.changed_fields.grid.update_docfield_property(
					"field_name",
					"options",
					options
				);
			} catch (e) {
				// Ignore dynamic option update errors.
			}

			let grid_df = frm.fields_dict.changed_fields.grid.docfields.find(
				(d) => d.fieldname === "field_name"
			);
