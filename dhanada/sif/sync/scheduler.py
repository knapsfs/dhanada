...
def sync_nav_performance(dry_run: bool = False):
	...
	try:
		...
		log_sync_completed(
			duration_seconds=round(duration, 2), stats=importer.stats, validation_errors=validation_errors
		)
...
def sync_scheme_details(dry_run: bool = False):
	...
	try:
		...
		validation_errors = mapper.validator.errors
		frappe.logger("sif_sync").info(
			f"Finished mapping data. Found {len(validation_errors)} validation errors."
		)
