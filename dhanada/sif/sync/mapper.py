...
class DataMapper:
	...
	def _derive_scheme_type(self, fund_type_desc: str) -> str | None:
	...
	def _extract_plans(self, plans_dict: dict, sebi_code: str, dataset: SyncDataset):
		...
		for plan_type_key, type_dict in plans_dict.items():
			...
			for option_key, option_val in type_dict.items():
				...
				if not isinstance(option_val, dict):
					continue
				...
				for sub_opt_key, sub_val in option_val.items():
					...
					if isinstance(sub_val, list):
						for rec in sub_val:
							self._create_plan_record(
								rec, sebi_code, mapped_type, mapped_option, mapped_sub, dataset
							)
						continue
					...
					if "isin_code" in sub_val:
						self._create_plan_record(
							sub_val, sebi_code, mapped_type, mapped_option, mapped_sub, dataset
						)
	...
	def _parse_managers(self, raw_managers_list: list[dict], dataset: SyncDataset) -> list[SchemeFundManager]:
		...
		date_pattern = re.compile(
			r"(\d{2}-[a-zA-Z]{3}-\d{4}|\d{2}-\d{2}-\d{4}|[a-zA-Z]+\s+\d{2},\s+\d{4}|\d{4}-\d{2}-\d{2}(\s+\d{2}:\d{2}:\d{2})?)"
		)

		# Pattern to capture trailing portions like "(Overseas portion)" or "Debt Portion"
		portion_extract_pattern = re.compile(
			r"(?i)(\([^)]*portion\)|Debt\s+Portion|Equity\s+Portion|Arbitrage\s+Portion)"
		)

		# Pattern to strip labels and titles from the front or within the name
		label_pattern = re.compile(
			r"(?i)(Debt|Equity|Arbitrage)\s+Portion\s*[:\-]?\s*|\(for.*?portion\)|\bFM\s*[-_]?\s*\d+\s*[:\-]?\s*|^\d+[\.\-\)]?\s+"
		)
		...
		for fm in raw_managers_list:
			...
			from_chunks = [
				c.strip()
				for c in re.split(r";|,|\band\b|&|\n", str(raw_from), flags=re.IGNORECASE)
				if c.strip()
			]
			from_dates = []
	...
	def _create_plan_record(
		self, node: dict, sebi_code: str, p_type: str, p_opt: str, p_sub: str | None, dataset: SyncDataset
	):
		...
		for raw_scheme in raw_data.get("scheme_details", []):
			if self.validator.validate_amfi_scheme_details(raw_scheme):
				...
				investment_strategy = self._derive_investment_strategy(
					raw_scheme.get("fund_type", ""), raw_scheme.get("category", "")
				)
					...
					if isinstance(alloc, dict):
						allocations.append(
							SchemeAllocation(
								...
								minimum_allocation_percentage=self._parse_float(
									alloc.get("minimum_percentage")
								),
								maximum_allocation_percentage=self._parse_float(
									alloc.get("maximum_percentage")
								),
							)
						)
