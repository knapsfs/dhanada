...
class GitHubClient:
	def __init__(self):
		self.repo_url = frappe.conf.get(
			"sif_sync_github_repo_url", "https://github.com/Satyam4755/AMFI_Fetcher"
		)
	...
	def fetch_scheme_details(self) -> list[dict[str, Any]]:
		...
		for file_info in json_files:
			try:
				...
				skipped += 1
				log_error(
					f"Failed to fetch or parse scheme detail file {file_info.get('name')}: {e}", exc_info=True
				)
	...
	def fetch_performance(self) -> list[dict[str, Any]]:
		...
		for file_info in json_files:
			try:
				...
				skipped += 1
				log_error(
					f"Failed to fetch or parse performance file {file_info.get('name')}: {e}", exc_info=True
				)
