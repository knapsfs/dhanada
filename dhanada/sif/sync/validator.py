...
class DataValidator:
	...
	def log_error(self, entity_type: str, identifier: str, message: str):
		self.errors.append({"entity_type": entity_type, "identifier": identifier, "error": message})
	...
	def validate_amfi_nav(self, raw_nav: dict[str, Any]) -> bool:
		...
		if not self._is_valid_date(raw_nav.get("nav_date")):
			self.log_error(
				"NAV", str(sif_code), f"Invalid date format for nav_date: {raw_nav.get('nav_date')}"
			)
			return False
