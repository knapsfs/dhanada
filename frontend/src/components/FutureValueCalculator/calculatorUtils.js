...
export const calculateRequiredInvestment = ({ targetFv, pv, annualRate, years, frequency, timing }) => {
  const isMonthly = frequency === 'monthly';
  const n = isMonthly ? years * 12 : years;
  const r = isMonthly ? annualRate / 100 / 12 : annualRate / 100;
  ...
  const calcData = calculateFutureValue({ pv, pmt: roundedPmt, annualRate, years, frequency, timing });
...
export const generateChartData = ({ calcMode, targetFv, pv, pmt, annualRate, years, frequency, timing }) => {
  const labels = [];
  const investedData = [];
  const growthData = [];
  const fvData = [];
  ...
  let actualPmt = pmt;
  if (calcMode === 'pmt') {
    const req = calculateRequiredInvestment({ targetFv, pv, annualRate, years, frequency, timing });
