...
export function cleanNavHistory(rawList) {
  ...
  return rawList
    .map(item => {
      ...
      const val = typeof item.nav === 'number' ? item.nav : parseFloat(String(item.nav).replace(/,/g, ''))
      return {
        date: d,
...
export function isPeriodAvailable(periodId, cleanHistory, perfData) {
  ...
  if ((rawVal === null || rawVal === undefined || rawVal === 'N/A') && (periodId === 'Since Inception' || periodId === 'Since Launch')) {
  ...
  if (rawVal === null || rawVal === undefined || rawVal === 'N/A' || isNaN(parseFloat(String(rawVal).replace('%', '')))) {
    return false
  }
...
export function generatePerformanceTable(perfData, historicalNav) {
  ...
  const tableDefinitions = [
    ...
    { periodId: 'Since Inception', label: 'Since Inception', keys: ['since_inception', 'since_launch'] },
  ]
  ...
  for (const def of tableDefinitions) {
    if (isPeriodAvailable(def.periodId, cleanHistory, perfData)) {
      ...
      if ((rawVal === null || rawVal === undefined || rawVal === 'N/A') && def.periodId === 'Since Inception') {
