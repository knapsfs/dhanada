...
const CATEGORY_GUIDES = {
	"large cap":
		"Large-cap funds invest in bigger, more established companies and are often used for core long-term equity allocation.",
	"flexi cap":
		"Flexi-cap funds can move across large, mid, and small caps, giving the fund manager more freedom.",
...
const MARKET_NEWS = [
	{
		...
		summary:
			"In this demo market snapshot, diversified large-cap exposure is holding up better than higher-volatility segments.",
	},
	{
		headline: "Debt allocations are helping reduce portfolio swings",
		summary:
			"Hybrid and short-duration debt categories are useful when investors want smoother movement with reasonable liquidity.",
	},
	{
		headline: "SIP discipline still matters more than short-term noise",
		summary:
			"For long-term goals, staying consistent with SIPs often matters more than trying to time every market move.",
	},
];
...
function findCity(query) {
	const text = normalizeText(query);
	return (
		Object.keys(DISTRIBUTORS).find((city) => city !== "default" && text.includes(city)) ||
		"default"
	);
}
...
export function getNAV(query) {
	...
	if (!fund) {
		return {
			status: "missing",
			message:
				"Please share the scheme name. In this demo, I can show sample NAV data for the listed funds.",
			availableFunds: FUNDS.map((item) => item.name),
		};
	}
...
export function getAMC(query) {
	...
	if (!amc) {
		return {
			status: "missing",
			message:
				"Please share the AMC name or fund name. I can explain the fund house and its strengths.",
...
export function getPerformance(query) {
	...
	if (!fund) {
		return {
			status: "missing",
			message:
				"Please share the scheme name. I can show sample 1Y, 3Y, and 5Y performance for the demo funds.",
			availableFunds: FUNDS.map((item) => item.name),
		};
	}
...
export function getRecommendation(profile = {}) {
	...
	if (risk.includes("conservative") || (horizonYears > 0 && horizonYears <= 3)) {
		...
		rationale.push(
			"Shorter or steadier goals usually need lower volatility than pure equity."
		);
	} else if (risk.includes("moderate") || risk.includes("balanced")) {
		suggestions.push(FUNDS.find((fund) => fund.name === "Cedar Balanced Advantage Fund"));
		suggestions.push(FUNDS.find((fund) => fund.name === "Horizon Bluechip Fund"));
		rationale.push(
			"A balanced mix can pair smoother movement with long-term growth potential."
		);
	} else {
	...
	const uniqueSuggestions = suggestions.filter(
		(fund, index) =>
			fund && suggestions.findIndex((item) => item?.name === fund.name) === index
	);
