...
files.forEach((file) => {
  ...
  // Find solid imports
  const solidMatch = content.match(
    /import\s+\{([^}]+)\}\s+from\s+['"]@fortawesome\/free-solid-svg-icons['"]/
  );
  if (solidMatch) {
    const icons = solidMatch[1]
      .split(",")
      .map((i) => i.trim())
      .filter((i) => i);
    icons.forEach((icon) => {
  ...
  const brandMatch = content.match(
    /import\s+\{([^}]+)\}\s+from\s+['"]@fortawesome\/free-brands-svg-icons['"]/
  );
  if (brandMatch) {
    const icons = brandMatch[1]
      .split(",")
      .map((i) => i.trim())
      .filter((i) => i);
    icons.forEach((icon) => {
