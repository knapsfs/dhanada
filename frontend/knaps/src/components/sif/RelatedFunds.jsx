    ...
    <section ref={ref} className="py-12 bg-[#f7f9fc]">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div
          ...
          <Link to="/sif" className="flex items-center gap-1.5 text-sm font-semibold text-[#032e92] hover:text-[#021d63] transition-colors">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {funds.map((fund, i) => (
            <motion.div
              key={fund.id}
              onClick={() => fund.id && navigate(`/sif/${encodeURIComponent(fund.id)}`)}
